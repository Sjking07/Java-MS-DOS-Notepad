...
public class Notepad extends JFrame implements ActionListener {
  ...
  private MenuBar menuBar = new MenuBar(); // first, create a MenuBar item
	...
	public Notepad() {
		...
		this.setTitle("MS-DOS Style Notepad Beep Beep lol"); // set the title of the window

		setDefaultCloseOperation(EXIT_ON_CLOSE); // set the default close operation (exit when it gets closed)
		
		Color customColor = new Color(0,127,80); // Custom MS-DOS jTextArea Color
		
		this.textArea.setFont(new Font("MONOSPACED", Font.BOLD, 16)); // set a default font for the TextArea
		
		textArea.setForeground(customColor); // Setting the font color
		
		this.getContentPane().setLayout(new BorderLayout()); // the BorderLayout bit makes it fill it automatically

		this.getContentPane().add(textArea).setBackground(Color.BLACK);
		...
		this.menuBar.setFont(new Font("MONOSPACED", Font.PLAIN, 12));
	...
	private Object setColor(Color white) {
		return null;
	}
